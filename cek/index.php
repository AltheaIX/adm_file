...
<body>
	...
	<div class="col-md-8">
		<h3 class="text-primary">DB BPSDMP KOMINFO MANADO</h3>
	...
	<table class="table table-striped table-bordered" style="margin-bottom: 0">
		...
		<tbody>
			<?php
				...
				$query = $conn->prepare("SELECT * FROM log_saya WHERE (`name` LIKE '%$keyword%' or `description` LIKE '%$keyword%') AND kat_id = 2 AND status > 0 ORDER BY Id DESC LIMIT 20");
				$query->execute();
...
?>
		...
		<tbody>
            ...
            $count = 1;
			    ...
			    $query = $conn->prepare("SELECT * FROM log_saya WHERE (`name` LIKE '%$keyword%' or `description` LIKE '%$keyword%') AND kat_id = 2  AND status > 0 ORDER BY Id DESC LIMIT 10");
				$query->execute();
