<?php
	...
	if($op == "tambah"){
        ...
        try {
          $sql = "INSERT INTO aset_lancar SET  
            nama = :nama,
			kat_id = :kat_id"
          ;
        ...
        if($stmt){		
			echo "<script>alert('Berhasil Tambah'); document.location.href=('../view/m_aset')</script>";
		}else{
			echo "<script>alert('Gagal Tambah'); document.location.href=('../view/m_aset')</script>";
		}
		...
		$sql = "DELETE FROM aset_lancar WHERE id = :id";
		$stmt = $conn->prepare($sql);
		$stmt->bindParam(':id', $id);
		$stmt->execute();
		
		if($stmt){		
			echo "<script>alert('Berhasil Menghapus'); document.location.href=('../view/m_aset/')</script>";
		}else{
			echo "<script>alert('Gagal Menghapus'); document.location.href=('../view/m_aset/')</script>";
		}
